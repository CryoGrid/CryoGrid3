function CryoGrid3_lake_batch(SETUP)
% CryoGRID3
...
%    PARA.technical.syncTimeStep = 12 ./ 24.0 ;          % output time step in [days] - here three hours
   ...
   %tsvd PARA.technical.subsurfaceGrid = [[0:0.02:4], [4.1:0.1:10], [10.2:0.2:20], [21:1:30], [35:5:50], [60:10:100], [200:100:1000]]'; % the subsurface K-grid in [m]
    ...
    PARA.location.initial_altitude=0.0;
...
%tsvd  extended for lateral switched off
    ...
    % switches for lateral processes
        ...
        %tsvd PARA = get_parallel_variables( PARA );
...
%nnn comment out for single mode...
    ...
    dE_dt_lateral_j_cor = zeros( length(GRID.general.cT_grid), 1);    % cell-wise heat flux in [W/m^3]? of all workers to worker index, corrected for interp bias
    xxx = zeros( length(GRID.general.cT_grid), 1);    % cell-wise heat flux in [W/m^3]? of all workers to worker index, corrected for interp bias
    ...
    iSaveSettings(  [ saveDir '/' run_number '/' run_number '_settings.mat'] , FORCING, PARA, GRID) %tsvd non-interpolated forcing data saved!
...
%original       SEB.dE_dt = SEB.dE_dt_cond + SEB.dE_dt_SEB;
                SEB.dE_dt = SEB.dE_dt_cond + SEB.dE_dt_SEB + dE_dt_lateral.*GRID.general.K_delta; %ccc zzz *kdelta?
        ...
        timestep = min( [ max( [ min( [ 0.5 * nanmin(GRID.general.K_delta.^2 .* c_cTgrid ./ k_cTgrid ./ (GRID.soil.cT_domain + GRID.snow.cT_domain ) ) ./ (24.*3600), ...
                    PARA.technical.targetDeltaE .* nanmin(abs(GRID.general.K_delta ./ SEB.dE_dt ) ) ./ (24.*3600), ...  % ttt zzz  modify to prevent very small timesteps if lake ice and heat exchange with NL ...
                    PARA.technical.maxTimestep ] ), ...
                    PARA.technical.minTimestep ] ), ...
                    TEMPORARY.outputTime-t ] );                
                    %PARA.technical.targetDeltaE .* nanmin(abs(GRID.general.K_delta ./ SEB.dE_dt.*(~GRID.lake.ice.cT_domain)  )) ./ (24.*3600), ...  % ttt zzz  modify to prevent very small timesteps if lake ice and heat exchange with NL ...
        ...
        if PARA.modules.lateral
            if t==TEMPORARY.syncTime %communication between workers
                ...
                if PARA.modules.exchange_heat
                   ...
                   % zzz not needed!?     dE_dt_lateral_j_cor = zeros( length(GRID.general.cT_grid), 1);  %in [J/m^3/s]                       
                   %     xxx = zeros( length(GRID.general.cT_grid), 1);  %in [J/m^3/s]                       
                        ...
                        % now down after correction   heat_fluxes(j) = nansum( dE_dt_lateral_j .* GRID.general.K_delta );  % in [J / m^2 s ], only for tracking the overall heat fluxes
                        %                             dE_dt_lateral = dE_dt_lateral + dE_dt_lateral_j;    % sum up contributions from all realizations
                            end
                        end
%tsvd correct for biases in dE_dt_lateral (due to interpolation?), by using mean value for pairwise heat exchange
                        labBarrier();
                        for j=1:number_of_realizations
                            if j~=index
                                %yyy2=dE_dt_lateral_j
                                labSend( nansum(dE_dt_lateral_j), j, 2);
                            end
                        end
                        for j=1:number_of_realizations
                            if j~=index
                                xxx = labReceive(j, 2);  %ttt rm xxx yyy .... if not needed...
                                %dE_dt_lateral_j_cor = -xxx/nansum(dE_dt_lateral_j) * dE_dt_lateral_j; % scale...xxx (fluxes have opposite sign)
                                dE_dt_lateral_j_cor = (-xxx+nansum(dE_dt_lateral_j))/(2*nansum(dE_dt_lateral_j)) * dE_dt_lateral_j; % scale dE_dt lateral to mean value between workers (fluxes have opposite sign)
                                heat_fluxes(j) = nansum( dE_dt_lateral_j_cor .* GRID.general.K_delta );  % in [J / m^2 s ], only for tracking the overall heat fluxes
%                               % dE_dt_lateral = dE_dt_lateral + dE_dt_lateral_j;    % sum up contributions from all realizations
                                % dE_dt_lateral_cor = dE_dt_lateral_cor + dE_dt_lateral_j_cor;    % sum up contributions from all realizations  -  here on two workers...
                                dE_dt_lateral = dE_dt_lateral_j_cor; % if more than two workers, need to sum up here.... xxx zzz
                            end
                        end                       
                     end
                 end
                ...
                if PARA.modules.exchange_water
                    ...
                    if precondition_waterExchange
                        ...
                        for j=1:number_of_realizations
                            if j~=index
                                labSend( PACKAGE_waterExchange, j, 3);
                            end
                        end
                        for j=1:number_of_realizations
                            if j~=index
                                PACKAGE_waterExchange_j = labReceive(j, 3);
