function CryoGrid3_function_spinup_fromSavedState( spinupFile, startDate, endDate )
...
% -------------------------------------------------------------------------
    ...
    if isempty(spinupFile)
        ...
        PARA.soil.externalWaterFlux=extFlux;%-2e-3;  %external water flux / drainage in [m/day]
        ...
        PARA.soil.waterTable=waterTable;              % depth at which a water table will form [m] - above excess water is removed, below it pools up  
        ...
        PARA.snow.maxSnow= [maxSnow] ;%0.2       % maximum snow depth that can be reached [m] - excess snow is removed in the model - if empty, no snow threshold
    ...
    %                     PARA.soil.waterTable, PARA.soil.externalWaterFlux, PARA.soil.fieldCapacity ] );
        ...
        wc(wc<PARA.soil.residualWC)=PARA.soil.residualWC;   % this fixes a bug which allowed wc<residualWC
        GRID.soil.cT_water = wc;
        GRID = initializeSoilThermalProperties(GRID, PARA);

        % here one could optionally change the forcing settings (rainFrac, snowFrac, etc)
        [FORCING, success]=load_forcing_from_file(PARA); % load FORCING mat-file
        if ~success
            warning('A problem with the Forcing occured.');
        end
        ...
        PARA.technical.starttime = startDate; %t; %take the end time from the spinup run as start time
        %PARA.technical.endtime = endDate;

        run_number = sprintf( [ 'SPINUP_' datestr( PARA.technical.starttime, 'yyyymmdd' ) '-' datestr(PARA.technical.endtime, 'yyyymmdd' ) '_stratSam_rf%d_sf%d_maxSnow%0.1f_snowDens=%0.1f_wt%0.1f_extFlux%0.4f_fc%0.2f' ], ...
                              [ PARA.forcing.rain_fraction, PARA.forcing.snow_fraction, PARA.snow.maxSnow, PARA.snow.rho_snow, ...
                              PARA.soil.waterTable, PARA.soil.externalWaterFlux, PARA.soil.fieldCapacity ] );
