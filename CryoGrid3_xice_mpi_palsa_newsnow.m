...
if SETUP.flag==0;
    number_of_realizations=2;  % <------ Number of realization !!!!!!!!!!!!!!!!!!!!!!!!!!!!!
else
...
if SETUP.flag==0;
    run_number= [datestr(date,'yymmdd') '_2w100y_TS_setup31'];
...
%% Start parallel routine

spmd
    index=labindex;   % number identifying the process; change this to e.g. 1 for single realization (non-parallel) run
    
    if SETUP.flag % To restart from a certain year
        [T, wc, t, SEB, PARA, GRID ] = iLoadFromState( ['runs' SETUP.slash SETUP.run_name_old SETUP.slash SETUP.run_name_old '_realization' num2str(labindex) '_finalState' num2str(SETUP.restartyear) '.mat']);
        % [ startFromRunDir '/' startFromRunName '/' startFromRunName '_realization' num2str(labindex) '_finalState' num2str(startFromRunYear) '.mat' ] );
        ...
        PARA.ensemble.geomSetup=31; % Numbver of the geometrical Setup
