function [GRID] = updateGRID_excessiceInfiltration(meltwaterGroundIce, GRID)
    ...
    if GRID.soil.cT_organic(1)+GRID.soil.cT_mineral(1)<=1e-6    % upper soil cell pure air/water
        ...
        cT_waterBody = GRID.soil.cT_organic+GRID.soil.cT_mineral<=1e-6; % Cells of pure air/water
        GRID.lake.cT_domain(logical(GRID.air.cT_domain+GRID.snow.cT_domain)) = 0; % Cell with either air or snow are not lake
        GRID.lake.cT_domain(GRID.soil.cT_domain) = cT_waterBody; % Use localization just before
