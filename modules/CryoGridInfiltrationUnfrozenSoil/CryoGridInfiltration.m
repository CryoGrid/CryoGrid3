...
% only infiltrate if no snow, unfrozen soil exists, tsvd: no-LAKE case added
if (isempty (GRID.snow.cT_domain_ub) && T(GRID.soil.cT_domain_ub)>0 ) && isempty (GRID.lake.water.cT_domain_ub) && isempty (GRID.lake.ice.cT_domain_ub)   % no snow cover and uppermost grid cell unfrozen, no lake
...
if sum(double(wc~=GRID.soil.cT_water & T(GRID.soil.cT_domain)<=0))>0  %zzz jjj  should be within if loop line 6...!?
