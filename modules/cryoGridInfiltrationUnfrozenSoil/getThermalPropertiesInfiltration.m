function [c_temp, k_temp, k_eff, lwc_temp] = getThermalPropertiesInfiltration(T, wc, c_temp, k_temp, k_eff, lwc_temp, GRID, PARA)
    ...
    assert(sum(imag(k_temp))==0,'getThPropInf : k_temp is complex at flag 0')
    assert(sum(imag(k_eff))==0,'getThPropInf : k_eff is complex at flag 0')

    %------- unused grid cells --------------------------------------------
    c_temp(GRID.air.cT_domain) = PARA.constants.c_a;
    k_temp(GRID.air.cT_domain) = PARA.constants.k_a;
    
    assert(sum(imag(k_temp))==0,'getThPropInf : k_temp is complex at flag 1')
    ...
    assert(sum(imag(k_temp))==0,'getThPropInf : k_temp is complex at flag 2')
    ...
    assert(sum(imag(k_temp))==0,'getThPropInf : k_temp is complex at flag 2.5')
    ...
    assert(sum(imag(k_temp))==0,'getThPropInf : k_temp is complex at flag 3')
    ...
    assert(sum(imag(k_eff))==0,'getThPropInf : k_eff is complex at flag 1')
             
    k_eff(2:end-1) = k_eff(2:end-1).^(-0.5);
    assert(sum(imag(k_eff))==0,'getThPropInf : k_eff is complex at flag 2')

    k_eff(1)     = k_temp(1);
    k_eff(end)   = k_temp(end);
    assert(sum(imag(k_eff))==0,'getThPropInf : k_eff is complex at flag 3')
    %------ correct upper most value below air-domain ---------------------
    k_eff(GRID.air.K_domain_lb+1) = k_temp(GRID.air.K_domain_lb+1);
    assert(sum(imag(k_eff))==0,'getThPropInf : k_eff is complex at flag 4')