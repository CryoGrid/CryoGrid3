...
%    p_area = area( ts, [ soil_level, terrain_level-soil_level, surface_level-terrain_level] );
    p_area = area( ts, [ soil_level, terrain_level-soil_level, surface_level-terrain_level],-10); % baseline at -10m
    ...
    axis( [ ts(1) ts(end) min(soil_level)-7.5 max(surface_level)+0.5 ] );
    %ax=gca;
