...
%tsvd    OUT.location.area = [OUT.location.area; PARA.location.area];
    ...
    if PARA.modules.lateral
        ...
        %OUT.lateral.dE_cell = cat( 3, OUT.lateral.dE_cell, TEMPORARY.dE_cell_lateral );    % matrix containing cell-wise, accumulated lateral heat fluxes in [J/m^3] to the current worker  tsvd: J/m2!
...
%        if(labindex<10)
%             iSaveOUT( [ saveDir '/'  run_number '_out' datestr(t,'yyyy') '_T0' num2str(labindex)  '.mat' ], OUT);
%             iSaveState( [ saveDir '/'  run_number '_finalState' datestr(t,'yyyy') '_T0' num2str(labindex)  '.mat' ], T, wc, t, SEB, PARA, GRID);
%             iPlotAltitudes( [ saveDir '/'  run_number '_altitudes' datestr(t,'yyyy') '_T0' num2str(labindex)  '.png' ], OUT, PARA);
%         else
            iSaveOUT( [ saveDir '/'  run_number '_out' datestr(t,'yyyy') '_T' num2str(labindex)  '.mat' ], OUT);
            iSaveState( [ saveDir '/'  run_number '_finalState' datestr(t,'yyyy') '_T' num2str(labindex)  '.mat' ], T, wc, t, SEB, PARA, GRID);
            iPlotAltitudes( [ saveDir '/'  run_number '_alts' datestr(t,'yyyy') '_T' num2str(labindex)  '.png' ], OUT, PARA);
%         end
        OUT = generateOUT();  
